					...
					if(modeSelection.equals("create")){
						...
						if(args.length != 4){
							...
							if(!(objCH instanceof PNCondition)){
								a.sendMessage("§4Je kan geen PNCondition kopiëren van een niet-PNCondition");
								return currentArray;
							}
							PNCondition newObject;
							try{
							...
							if(currentArray == null || currentArray.length == 0){
								currentArray = new PNCondition[1];
								currentArray[0] = newObject;
								a.sendMessage("§eDe gekopiëerde PNCondition is de enige PNCondition in deze PulserNotifStandard. Zijn index is dus §9#0§e.");
								affectedCondition.notifyChange();
							}else{
								int index;
								for(index = 0; index < currentArray.length; index++){
									if(currentArray[index] == null){
										a.sendMessage("§eEen vrije plaats voor de gekopiëerde PNCondition is gevonden op index §9#" + index + "§e.");
										break;
									}
								}
								if(index < currentArray.length){
									...
									a.sendMessage("§eDe lengte van de PNConditions lijst wordt uitgebreid van " + currentArray.length + " naar " + (currentArray.length + 1) + " om plaats te voorzien voor de gekopiëerde PNCondition.");
									PNCondition[] newArray = new PNCondition[currentArray.length + 1];
									System.arraycopy(currentArray, 0, newArray, 0, currentArray.length);
									newArray[newArray.length - 1] = newObject;
									currentArray = newArray;
