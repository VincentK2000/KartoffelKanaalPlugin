						...
						if(modeSelection.equals("create")){
							...
							if(args.length != 4){
								...
								if(!(objCH instanceof PNTech)){
									a.sendMessage("§4Je kan geen PNTech kopi§ren van een niet-PNTech");
									return true;
								}
								PNTech newObject;
								try{
								...
								if(this.technics == null || this.technics.length == 0){
									this.technics = new PNTech[1];
									this.technics[0] = newObject;
									a.sendMessage("§eDe gekopi§erde PNTech is de enige PNTech in deze PulserNotifStandard. Zijn index is dus §9#0§e.");
									this.notifyChange();
								}else{
									int index;
									for(index = 0; index < this.technics.length; index++){
										if(this.technics[index] == null){
											a.sendMessage("§eEen vrije plaats voor de gekopi§erde PNTech is gevonden op index §9#" + index + "§e.");
											break;
										}
									}
									if(index < this.technics.length){
										...
										a.sendMessage("§eDe lengte van de PNTechs lijst wordt uitgebreid van " + this.technics.length + " naar " + (this.technics.length + 1) + " om plaats te voorzien voor de gekopi§erde PNTech.");
										PNTech[] newArray = new PNTech[this.technics.length + 1];
										System.arraycopy(this.technics, 0, newArray, 0, this.technics.length);
										newArray[newArray.length - 1] = newObject;
										this.technics = newArray;
